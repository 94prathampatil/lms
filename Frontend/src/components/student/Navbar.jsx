...
const Navbar = () => {
  ...
  const { openSignIn } = useClerk()     // will Open Clerk Signup form 
  const { user } = useUser()
  ...
  return <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36
    border-b border-gray-500 py-4 ${isCourseListPage ? "bg-white" : "bg-cyan-100/70"}`}>
      ...
      <button>
          {user ? <UserButton/> : <img onClick={() => openSignIn()} src={assets.user_icon} alt="" />}
        </button>
      </div>
    </div>;
};
